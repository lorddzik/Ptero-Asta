...
const Limit = ({ limit, children }: { limit: string | null; children: React.ReactNode }) => (
    <>
        {children}
        <span className={'ml-1 text-[70%] select-none'} style={{ color: 'var(--neo-text-muted)' }}>/ {limit || <>&infin;</>}</span>
    </>
);
    ...
    return (
        <div className={classNames('grid grid-cols-6 gap-2 md:gap-4', className)}>
            ...
            <StatBlock icon={faCloudDownloadAlt} title={'Network (Inbound)'}>
                {status === 'offline' ? <span style={{ color: 'var(--neo-text-muted)' }}>Offline</span> : bytesToString(stats.rx)}
            </StatBlock>
            <StatBlock icon={faCloudUploadAlt} title={'Network (Outbound)'}>
                {status === 'offline' ? <span style={{ color: 'var(--neo-text-muted)' }}>Offline</span> : bytesToString(stats.tx)}
            </StatBlock>
        </div>
    );
};
